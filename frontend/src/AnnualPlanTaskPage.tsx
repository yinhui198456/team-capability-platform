...
import { getAnnualPlan, generatePlanItems, getAnnualPlanEligibility, type AnnualPlan, type PlanItem, type LearningTask } from './planning'
import { mockPlan, mockTasks, mockLogs, mockEvidences, isMockEnabled } from './__fixtures__/annualPlanMock'

const MONTHS = Array.from({ length: 12 }, (_, i) => i + 1)
const STATUS_LABELS: Record<string, string> = { 未开始: '未开始', 进行中: '进行中', 已完成: '已完成', 延期: '延期', 暂停: '暂停', 取消: '取消' }

function hours(v: string | null | number): number { const n = typeof v === 'number' ? v : Number(v ?? 0); return Number.isFinite(n) ? n : 0 }

type TaskDetail = { task: LearningTask; logs: any[]; evidences: any[] }

function statusClass(st: string) { return st === '已完成' ? s.statusDone : st === '延期' ? s.statusOverdue : st === '进行中' ? s.statusActive : s.statusTodo }

function evBadge(ev: any) {
  if (!ev) return null
  const cls = ev.status === '通过' ? s.evidencePass : ev.status === '待 Review' ? s.evidencePending : s.evidenceNeedMore
...
export function AnnualPlanTaskPage() {
        ...
        if (isMockEnabled()) {
          if (!c) {
            ...
            for (const t of Object.values(mockTasks)) {
              tm[t.plan_item_id] = { task: t, logs: mockLogs[t.id] ?? [], evidences: mockEvidences[t.id] ?? [] }
            }
            setTasks(tm)
          }
        } else {
  ...
  async function handleGenerate() {
    ...
    try { await generatePlanItems(); const p = await getAnnualPlan(year); setPlan(p) }
    catch (err: any) { setError(err.message ?? '生成失败') }
    finally { setGenerating(false) }
  }

  const items = plan?.items ?? []
  const visibleItems = selectedMonth ? items.filter(i => i.target_month === selectedMonth) : items
  const totalEstimated = items.reduce((sum, i) => sum + hours(i.estimated_hours), 0)
  const totalActual = Object.values(tasks).reduce((sum, t) => sum + (t.task.actual_hours ?? 0), 0)
  ...
  return (
    <section className="page">
      <header className="page-heading">
        <div><p className="eyebrow">我的计划 / 年度闭环</p><h1>年度成长计划</h1><p className="muted">{year} 年度 · {plan?.plan_cycle ?? 12} 个月周期 · {plan?.status ?? '制定中'}</p></div>
        <div>
          {items.length === 0 && <button className="primary" onClick={handleGenerate} disabled={generating}>{generating ? '生成中…' : '生成计划项'}</button>}
          <a href="/growth/review/monthly" style={{ marginLeft: 'var(--space-3)' }}>查看月度复盘</a>
        </div>
      </header>
          ...
          return (
            <div className={s.planItem} key={item.id} data-testid="plan-item">
              <div className={s.planHeader} data-testid="plan-header" onClick={() => setExpandedId(isExpanded ? null : item.id)} role="button" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') { setExpandedId(isExpanded ? null : item.id) } }}>
                ...
                <span>{item.target_month ? `${item.target_month} 月` : '—'}</span>
                <span className={`${s.status} ${statusClass(st)}`}>{STATUS_LABELS[st] ?? st}</span>
                ...
                <div className={s.taskPanel} data-testid="task-detail-panel">
                  ...
                  {td.evidences.length > 0 && (
                    <div className={s.logSection}>
                      <h4>Evidence</h4>
                      {td.evidences.map((ev: any) => (
                        <div key={ev.id} className={s.logItem}>
                          <span className={s.logDate}>v{ev.version_number}</span>
                          <span className={s.logNote}>{ev.content?.slice(0, 60) ?? '—'}</span>
                          {evBadge(ev)}
                          {ev.evidence_link && <a href={ev.evidence_link} target="_blank" rel="noreferrer" style={{ fontSize: 'var(--text-xs)' }}>链接</a>}
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {td.logs.length > 0 && (
                    <div className={s.logSection}>
                      <h4>学习日志 ({td.logs.reduce((s: number, l: any) => s + l.actual_hours, 0)} h)</h4>
                  ...
                  {!td.evidences.length && !td.logs.length && <p className="muted" style={{ marginTop: 'var(--space-2)' }}>暂无执行记录。前往学习任务页面添加。</p>}
                  <a href="/growth/tasks" style={{ display: 'inline-block', marginTop: 'var(--space-3)', fontSize: 'var(--text-sm)' }}>进入 Evidence 与学习日志</a>
                </div>
              )}
            </div>
          )
