...
const LEVEL_HELP =
  'P4–P8 为岗位等级要求，1–5 为能力项掌握度（1=入门, 3=独立, 5=专家）。建议起始等级来自能力模型，仅供参考。'
...
function priority(gap: number): '高' | '中' | '低' {
...
export function AssessmentGapPage() {
      ...
      try {
        if (isMockEnabled()) {
          if (!c) {
            ...
            setDetails(mockAssessment.details ?? [])
          }
        } else {
  ...
  async function handleCreate() {
    ...
    try {
      const { id } = await createAssessment(year)
  ...
  async function handleSubmit() {
    ...
    setMessage('')
    if (isMockEnabled()) {
      const sub = { ...mockAssessmentSubmitted, details }
      setAssessment(sub)
      setDetails(details)
  ...
  function startEdit(id: number | undefined, text: string) {
    setEditingId(id ?? null)
    setEditingText(text)
  }
  function commitEdit(index: number) {
    updateDetail(index, { evidence_note: editingText })
    setEditingId(null)
  }
  ...
  const filtered = useMemo(() => {
    ...
    if (activeDomain !== '全部')
      list = list.filter(
        (d) => (d.l1_code ?? d.l3_code.split('.')[0]) === activeDomain,
      )
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (d) =>
          d.l3_code.toLowerCase().includes(q) || (d.l3_name ?? '').includes(q),
      )
    }
    return list
  ...
  const grouped = useMemo(() => {
    ...
    for (const d of filtered) {
      const k = d.l1_code ?? d.l3_code.split('.')[0]
      if (!m.has(k)) m.set(k, [])
      m.get(k)!.push(d)
    }
    return [...m.entries()]
  ...
  function locateNextUnfilled() {
    ...
    setActiveDomain(l1)
    setTimeout(() => {
      const el = document.getElementById(`row-${item.id}`)
  ...
  return (
    <section className="page">
      ...
      {!canPlan && (
        <p className={s.planGateWarning}>
          Review 认可闭环后才可正式纳入年度计划。当前状态：{reviewLabel}。
        </p>
      )}
      {isEditable && unfilled > 0 && (
        <p className={s.submitWarning}>
          还有 {unfilled} 项未完成，请完成全部自评后再提交。
        </p>
      )}
        ...
        <div className={s.toolbar}>
          <div className={s.toolbarGroup}>
            ...
            <input
              ...
              placeholder="搜索 L3 编号/名称"
              aria-label="搜索能力项"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            <button className={s.locateBtn} onClick={locateNextUnfilled}>
              定位未填写项
            </button>
          </div>
        ...
        <div>
          {grouped.map(([code, items]) => (
            <div className={s.domainGroup} key={code}>
              <div
                className={s.domainHeader}
                onClick={() =>
                  setActiveDomain(code === activeDomain ? '全部' : code)
                }
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
                ...
                aria-expanded={activeDomain === '全部' || activeDomain === code}
              >
                <span className={s.domainLabel} data-testid="domain-label">
                  {code === activeDomain ? '▾' : '▸'} {domainLabel(code)}
                </span>
                <span className={s.domainProgress}>
                  {items.filter(isFilled).length}/{items.length} 项
                </span>
              </div>
                      ...
                      <>
                        <tr className={rowCls} key={d.id} id={`row-${d.id}`}>
                          ...
                          <td>
                            <input
                              type="checkbox"
                              checked={d.plan_candidate ?? false}
                              onChange={(e) =>
                                updateDetail(gIdx, {
                          ...
                          <td>
                            {isEditable && (
                              <button
                                ...
                                onClick={() =>
                                  startEdit(d.id, d.evidence_note ?? '')
                                }
                                title="编辑自评依据"
                              >
                                {filled ? '✎' : '+'}
          ...
          {grouped.length === 0 && (
            <p className="muted">当前筛选条件下无匹配能力项。</p>
          )}
        </div>
          ...
          <aside className={s.gapSidebar} data-testid="gap-sidebar">
            ...
            <div className={s.gapBars}>
              <div className={s.gapBar}>
                <span>高 ({gs.high_priority})</span>
              </div>
              <div
                className={`${s.gapBarFill} ${s.fillHigh}`}
                style={{
                  width: `${gs.total_gaps ? (gs.high_priority / gs.total_gaps) * 100 : 0}%`,
                }}
              />
              <div className={s.gapBar}>
                <span>中 ({gs.medium_priority})</span>
              </div>
              <div
                className={`${s.gapBarFill} ${s.fillMedium}`}
                style={{
                  width: `${gs.total_gaps ? (gs.medium_priority / gs.total_gaps) * 100 : 0}%`,
                }}
              />
              <div className={s.gapBar}>
                <span>低 ({gs.low_priority})</span>
              </div>
              <div
                className={`${s.gapBarFill} ${s.fillLow}`}
                style={{
                  width: `${gs.total_gaps ? (gs.low_priority / gs.total_gaps) * 100 : 0}%`,
                }}
              />
            </div>
            {!canPlan && (
