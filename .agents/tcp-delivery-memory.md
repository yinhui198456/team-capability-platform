...
- 创建新 Issue worktree 的准入必须核对基础分支同时包含当前批准版本的 `AGENTS.md` 与本文件，并记录两者的 Git blob/提交来源；任一缺失或落后即停止创建，先用独立记忆 PR 把稳定规则合入基础分支。
- `/opt/personal-agent-workspace/team-capability-platform` 是运行/UAT checkout，不是项目记忆发布源。它处于 detached、脏状态或缺少记忆文件时，禁止从中提交；并行 Issue 需要尚未进入基础分支的稳定记忆时，从已提交的权威分支提取到独立记忆 PR。
