...
- 未授权 DB/环境变更时，若启动或检查会自动迁移，必须先停下并取得具体授权。红色 CI 只证明候选失败，不自动扩大文件、行为、环境或修复范围；例外必须先由当前有界合同明确授权。
- 监督消息的 `queued`、`received`、`replied`、`completed` 必须分开报告；Goal 暂无推进、缺少状态采集字段或正常等待用户不等于业务失败，不重复刷屏。监督者只联系本 Issue Sol，不旁路指挥 writer；监控收尾要核对过期监控，但停止、清理或恢复监控仍需独立操作授权。
