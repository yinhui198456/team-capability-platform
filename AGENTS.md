...
本节是 TCP 每个 Issue 七阶段交付流程的唯一规范正文，不得与 `docs/05_Development.md`、`docs/06_Roadmap.md` 的“项目迭代 0–7”混用。

1. 阶段1：范围与验收基线。
2. 阶段2：方案与原型确认。
3. 阶段3：最小实现合同与风险门禁。
4. 阶段4：红测先行实现与自测。
5. 阶段5：非 writer 专业 UI/UX 与合同评审。
6. 阶段6：精确候选版本部署到测试环境，完成 smoke、真实 Chrome 需求级操作及原型/视觉对照；这些结论分层记录。
7. 阶段7：用户最终 UAT；任何 Agent、自动化、Actions、测试或浏览器自验不得替代。

- Windows 与 Ubuntu 不维护两份七阶段正文；Ubuntu/worktree 通过 Git 获得根 `AGENTS.md`，Windows Monitor 只从当前权威分支读取本文件。
- #132 历史产物名“阶段1修订版/Stage-2评审包”只是历史标签，不得覆盖标准阶段定义；未有用户明确特例时一律按本节七阶段解释。
- 阶段切换由用户可见门禁决定；不得因已有原型、测试或 Agent 自述自动越级。
- 本次治理修复的状态说明（按用户明确要求）：#132 阶段1通过，进入阶段2；本轮仅先固化治理规则，不继续阶段2产出。
