...
- L3 编辑与生产 `NodeEditForm` 字段范围一致：代码和父 L2 只读，建议起始职级和输出类型为带可见标签的原生 select；时长使用 decimal mobile keyboard 的受控文本输入，可留空并在字段内联报告负数、指数、单位/文本或两位小数，保存不舍入。`16 小时` 示例预填为 `16`，展示仍只保留一次“小时”；遗留范围不解析。输出类型的 workbook 词汇为拟议值（并非正式枚举）。其余目录字段与既有资源关联可编辑；已发布 P4–P8 矩阵始终无编辑控件，Member 的 P5 目标不投射到 Leader 维护视图，并指向既有标准版本维护入口。
