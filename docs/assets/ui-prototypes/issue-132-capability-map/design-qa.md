...
- **Implementation：** `qa/prototype-review-package-1440x900.png`、`qa/prototype-member-1440x900.png`、`qa/prototype-leader-1440x900.png`、`qa/prototype-sticky-member-1440x900.png`、`qa/prototype-leader-edit-1440x900.png`、`qa/prototype-leader-l3-edit-upper-1440x900.png`、`qa/prototype-leader-l3-edit-lower-1440x900.png`、`qa/prototype-leader-save-1440x900.png`，DPR 1、CSS 1440×900。
...
- L3 编辑与生产 `NodeEditForm` 字段范围一致：代码和父 L2 只读，建议起始职级和输出类型为带可见标签的原生 select；时长使用 decimal mobile keyboard 的受控文本输入，可留空并在字段内联报告负数、指数、单位/文本或两位小数，保存不舍入。所有无歧义的数值小时 mock 值（如 `8 小时`、`16 小时`）预填为数值，展示仍只保留一次“小时”；遗留范围不解析。输出类型的 workbook 词汇为拟议值（并非正式枚举）。其余目录字段与既有资源关联可编辑；已发布 P4–P8 矩阵始终无编辑控件，Member 的 P5 目标不投射到 Leader 维护视图，并指向既有标准版本维护入口。
...
- P01/P02/P03/C01 Tab 点击无纵向 scroll drift；L3 的原生 select、时长 `1` / `1.5` 保存、起始职级范围保存后重开、`1.23` / `-1` / `1e2` / 文本 / 单位输入阻止保存且保留表单状态、只读发布矩阵、焦点循环及桌面页头基线均有精确断言。`prototype-leader-l3-edit-upper-1440x900.png` 和 `prototype-leader-l3-edit-lower-1440x900.png` 分别记录 Drawer 上下区域；后者包含内联错误态。
