# Issue #132 · 阶段 1 修订版高保真交互原型：阶段 2 评审包需求与验证矩阵

## 本轮已纳入的评审状态

- 同一入口以 `?role=member`（默认）和 `?role=leader` 提供只读与维护状态；没有页面内伪造角色切换。Member、Buddy、Leader、Admin 都可查看；只有 Leader 维护，Admin 未兼任 Leader 时保持只读。
- 顶栏统一显示年度、用户姓名和退出，不显示“数据范围”。Member 为 `Member User`；Leader 为 `Leader User`。侧栏按 `frontend/src/Layout.tsx` 的 Member / Leader 信息架构展示。
- Leader 有“标准版本维护”入口（仅提示该页不属于 #132）、L1/L2/L3 编辑入口、名称/启用状态/相应标准文本表单、取消/保存和仅原型会话内的成功反馈；查看 L3 与编辑 Drawer 分离。无创建、硬删除、持久化或后端调用。
- 本轮 Stage-2 反馈修订：L1 Tab 切换仅调整 Tab 容器横向可见性，不改变页面纵向滚动；Leader 页头的搜索输入与“标准版本维护”在同一控件基线；L3 编辑仅覆盖当前更新 API 的业务字段。
- 仅 L1 Tab 行 sticky；它有必要的白色背景和轻微阴影，`scroll-padding`/`scroll-margin` 保证深链和键盘目标不被遮挡，窄视口横向可滚动并保持当前 Tab 可见。
- 统一评审入口固定为 `docs/assets/ui-prototypes/prototype-v1/index.html`；`?collection=selected&page=M02` 等既有 URL 原样转给 `viewer.html`（保留原编译产物），CM01 提供 Member 与 Leader 两个链接。

## 决策完整性

结论：**修改后接受**。

- 接受目标：让能力地图页面状态与可分享 URL 始终一致，并统一搜索、深链接、刷新、后退/前进和键盘操作。
- 修改既有方法：`docs/04_UI.md` 只规定初始 L3 Hash 定位；#132 要求将同一规则扩展到用户主动选择的 L1/L2/L3，并监听浏览器历史变化。
- 不采用静态图片或只修搜索点击：二者都无法完整覆盖状态恢复、键盘和无效深链接。
- 不修改业务模型、权限、接口、数据库或 #201；阶段 1 修订版高保真交互原型仅形成可评审交互合同。

## URL 合同

统一格式为 `/capability/model#<CODE>`；本静态原型以 `index.html#<CODE>` 等价演示。

| 状态 | URL | 页面结果 | 历史行为 |
|---|---|---|---|
| 默认入口 | 无 Hash | 默认显示 P01，不伪造一次用户选择 | 保留入口历史 |
| 选择 L1 | `#P02` | 选中并聚焦 P02 域概览 | 新增一条历史记录 |
| 选择 L2 | `#P02.02` | 选中 P02、展开并聚焦 P02.02 | 新增一条历史记录 |
| 选择 L3 | `#P02.02.08` | 选中 P02、展开 P02.02、聚焦并高亮 L3；搜索定位不自动打开 Drawer | 新增一条历史记录 |
| 刷新 | 保持当前 Hash | 从 URL 完整恢复当前层级 | 不新增历史记录 |
| 后退/前进 | 浏览器提供的 Hash | 恢复对应 L1/L2/L3 与展开、焦点状态 | 不重写历史 |
| 同页 Hash 变化 | 任一有效 Hash | 立即同步页面，不要求刷新 | 由浏览器历史驱动 |
| 可识别域的无效路径 | 如 `#P02.02.99` | 进入 P02，上方明确提示路径不存在；不冒充某个 L2/L3 已选中 | 保留问题 Hash 便于修复/分享 |
| 完全无效路径 | 如 `#NOT-A-CODE` | 不选中任何域，显示未找到和恢复动作 | 恢复动作清除无效 Hash |

## 逐项验收

| ID | 来源 | 操作 | 可见结果 |
|---|---|---|---|
| A-01 | #132 | 从无 Hash 点击 P02 | 地址变为 `#P02`，页面和 Tab 同步为 P02 |
| A-02 | #132 | 点击或搜索选择 P02.02 | 地址变为 `#P02.02`，父域选中、L2 展开并获得焦点 |
| A-03 | #132 | 搜索选择 P02.02.08 | 地址变为 `#P02.02.08`，P02/P02.02/L3 同步，高亮目标但不自动打开 Drawer |
| A-04 | #132 | 刷新上一步页面 | 仍定位 P02.02.08，搜索输入可清空但 URL 选择不丢失 |
| A-05 | #132 | 连续选择 C01.01.01 → P02 → P02.02.08，再后退/前进 | 每一步地址与页面层级一致，不静默跳回旧项 |
| A-06 | #132 评论 | 地址栏从有效项改成另一有效 Hash | 页面立即同步，无需刷新 |
| A-07 | #52 | 搜索 L1/L2/L3 编号或名称 | 结果显示类型及所属 L1/L2；输入时不自动跳转 |
| A-08 | #162 | 搜索结果打开时按 ArrowDown/ArrowUp、Home/End、Enter、Escape | 活动项清晰；Enter 选择；Escape 关闭并保留查询；焦点无陷阱 |
| A-09 | #161 | 打开 `#P02.02.99` | 显示 P02 上下文和“路径不存在”，不静默显示 P01 |
| A-10 | #161 | 打开 `#NOT-A-CODE` | 显示页面级未找到；恢复动作清除无效 Hash 后进入 P01 |
| A-11 | #153 | 展开 L2 并切换 P4～P8 | 按钮只显示可扫描摘要；内联区显示更完整且非逐字重复的要求 |
| A-12 | docs/04_UI.md | 清除搜索 | 保留当前 URL 选择、能力域与 L2 展开状态 |
| A-13 | docs/04_UI.md | 点击 L3 行并关闭 Drawer | URL 定位到该 L3；Drawer 展示上下文；Escape/关闭后焦点返回触发行 |
| A-14 | docs/04_UI.md | 在 1440×900、1920×1080、1024×768 与 768×1024 查看 | 无页面级横向滚动、遮挡和文字碎裂；核心导航与搜索仍可操作 |
| A-15 | #132 approved feedback | 打开 `?role=member` 与 `?role=leader` | 两种可评审状态 URL 可分享；无页面内角色切换；顶栏和侧栏符合对应角色 |
| A-16 | #132 approved feedback | Leader 编辑 L1、L2、L3 | 仅 Leader 看见维护操作；独立编辑 Drawer 提供标签、启用、取消/保存和成功反馈 |
| A-17 | #132 approved feedback | 滚动后使用 Tab、深链或键盘焦点 | 仅 L1 Tab 粘附，目标未被遮挡；窄视口仍可操作 |
| A-18 | #132 approved feedback | 打开统一入口与原有 M02 wrapper | CM01 两条状态链接可打开；`?collection=selected&page=M02` 仍进入既有 viewer |
| A-19 | #132 Stage-2 feedback | 在同一滚动位置依次点击 P01 / P02 / P03 / C01 | sticky L1 行 top 与 `window.scrollY` 不漂移；窄视口仍将活动 Tab 保持在横向可见区 |
| A-20 | #132 Stage-2 feedback | Leader 桌面页头 | 搜索输入与“标准版本维护”按钮底部控件基线对齐；窄视口整洁纵向排列 |
| A-21 | docs/03_Data.md §3.7、生产 NodeEditForm | Leader 编辑 L3 | 只读代码/L2 上下文；可编辑名称、启用、建议起始职级（接受单一职级或范围，如 P4 或 P4–P5）、预计时长、预期输出、输出类型、原始学习材料、备注和既有资源关联；会话保存同步更新行与详情 |
| A-22 | docs/04_UI.md §4.6/§5 | 查看 L3 Drawer 的 P4–P8 | 已发布矩阵只读，明确说明仅可通过现有“标准版本维护”流程修改；Member 显示 P5 目标，Leader 仅显示中性的维护视图且没有目标高亮；L3 表单没有 P4–P8 编辑字段 |
| A-23 | #132 Stage-2 feedback | 在 L3 编辑 Drawer 使用键盘 | input、textarea、checkbox 都可进入 Tab 序；关闭和保存端点形成焦点循环 |
| A-24 | #132 L3 controls | Leader 编辑 L3 的建议起始职级、预计时长和输出类型 | 建议起始职级为原生 select，且仅含 P4、P5、P6、P7、P8、P4–P5、P5–P6、P6–P7、P7–P8、P6–P8、P5–P8；保留当前单值/范围。预计时长标签为“预计时长（小时）”，可为空，接受整数或最多一位小数；负数、指数、单位/文本、两位小数在原字段内联报错并阻止保存，不静默舍入。所有无歧义的数值小时 mock 值（如 `8 小时`、`16 小时`）在该表单预填为数值，详情始终只显示一次“小时”；其他遗留范围/格式不解析或迁移。输出类型为原生 select，保留既有示例“文档 / 演示”，并列出拟议 workbook 词汇：实操、方案设计、问题处理、经验沉淀、方法论沉淀、架构图+说明、实操+说明、实操+方案、实操+分析、实操+设计、实操+验证、Demo+交付物、POC 验证报告。 |

## 范围边界

- 不把搜索查询、Drawer 开关或所选 P4～P8 职级写入 URL；它们不是可分享的能力路径。
- 不新增路由、API、权限、业务角色或数据对象。
- 不改变 L2“能力标准/职级要求”、L3“达成路径/学习实践项”的正式语义。
- 不把原型中的示例文本或数量反向定义为生产数据。
- L3 表单不编辑已发布 P4–P8 掌握度目标，也不发明 `sort_order`；它们不在当前 L3 更新 API 中。发布矩阵只由独立的“标准版本维护”流程维护。
- 上述输出类型词汇只用于原型评审：它来自 workbook 的 `output_type` 有效值，workbook 同列仍含错位备注，因此它不是正式业务枚举；不据此修改数据模型或接口。遗留时长范围的转换规则尚未决定，本轮不迁移。
