...
本目录是 Issue #132 的阶段 1 修订版高保真交互原型，纳入供阶段 2 用户评审的阶段 2 评审包。它沿用当前能力地图视觉基线，以纯静态 HTML/CSS/JavaScript 演示 URL/Hash、L1/L2/L3 定位、搜索、浏览器历史、键盘、sticky L1 与 Leader 维护交互；不连接 API、数据库或运行环境，也不修改产品代码。
