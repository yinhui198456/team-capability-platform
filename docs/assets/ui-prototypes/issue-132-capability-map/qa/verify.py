...
def run():
    ...
    with sync_playwright() as playwright:
        ...
        page.goto(BASE_URL + "?role=leader#P01.01.01")
        page.locator('[data-l3-code="P01.01.01"] + [data-edit-code="P01.01.01"]').click()
        expect(page.locator('input[name="hours"]')).to_have_value("8")
        page.get_by_role("button", name="保存").click()
        expect(page.get_by_role("status")).to_contain_text("P01.01.01 已保存")
        page.goto(BASE_URL + "?role=leader#P02.02.08")
        ...
        for label in ["代码", "所属能力组", "名称", "建议起始职级", "预计时长（小时）", "预期输出", "输出类型", "原始学习材料", "备注"]:
        ...
        expect(start_level.locator("option")).to_have_text(["P4", "P5", "P6", "P7", "P8", "P4–P5", "P5–P6", "P6–P7", "P7–P8", "P6–P8", "P5–P8"])
        hours = edit_form.locator('input[name="hours"]')
        expect(hours).to_have_value("16")
        expect(hours).to_have_attribute("inputmode", "decimal")
        output_type = edit_form.locator('select[name="outputType"]')
        expect(output_type).to_have_value("文档 / 演示")
        expect(output_type.locator("option")).to_have_text(["文档 / 演示", "实操", "方案设计", "问题处理", "经验沉淀", "方法论沉淀", "架构图+说明", "实操+说明", "实操+方案", "实操+分析", "实操+设计", "实操+验证", "Demo+交付物", "POC 验证报告"])
        page.screenshot(path=evidence / "prototype-leader-l3-edit-upper-1440x900.png")
        ...
        page.keyboard.press("Tab")
        expect(output_type).to_be_focused()
        resource.focus()
        ...
        start_level.select_option("P5–P6")
        hours.fill("1")
        ...
        expect(page.locator('[data-l3-code="P02.02.08"]')).to_contain_text("P5–P6")
        expect(page.locator('[data-l3-code="P02.02.08"]')).to_contain_text("1 小时")
        page.locator('[data-l3-code="P02.02.08"] + [data-edit-code="P02.02.08"]').click()
        expect(start_level).to_have_value("P5–P6")
        expect(hours).to_have_value("1")
        hours.fill("1.5")
        edit_form.get_by_role("button", name="保存").click()
        expect(page.locator('[data-l3-code="P02.02.08"]')).to_contain_text("1.5 小时")
        page.locator('[data-l3-code="P02.02.08"] + [data-edit-code="P02.02.08"]').click()
        for invalid_hours in ["1.23", "-1", "1e2", "文本", "1 小时"]:
            hours.fill(invalid_hours)
            edit_form.get_by_role("button", name="保存").click()
            expect(page.get_by_role("alert")).to_contain_text("预计时长（小时）只能填写非负整数或一位小数。")
            expect(hours).to_have_value(invalid_hours)
            expect(hours).to_be_focused()
            expect(hours).to_be_in_viewport()
            expect(page.locator("#hours-error")).to_be_in_viewport()
            expect(page.locator(".edit-drawer")).to_be_visible()
        edit_form.locator(".form-actions").scroll_into_view_if_needed()
        page.screenshot(path=evidence / "prototype-leader-l3-edit-lower-1440x900.png")
        page.get_by_role("button", name="取消").click()
        page.locator('[data-l3-code="P02.02.08"]').click()
        dialog = page.get_by_role("dialog", name="P02.02.08")
