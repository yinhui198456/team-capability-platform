...
> ⚠️ 本节「提交自评时原子生成计划与任务」已由 Issue #187 故事合同第 2、3、6 条替代（改为「保存能力评级」「加入/移出提升计划草稿」「生成所选学习任务」三个独立动作），见下文「Issue #187 故事合同」。

## Issue #187 故事合同（第一批原型基线，2026-08-16 确认）

本节是 Issue #187 阶段 0 确认的**不可改写故事合同**（原型基线）。与上文既有规则冲突时，以本节为准；上文相关段落已就地标注「已替代」，原文保留供历史追溯。原型资产与打开方式见 `docs/assets/ui-prototypes/README.md`；页面矩阵见 `docs/04_UI.md` §4.9。
...
1. 能力评级允许任意部分逐项保存，不要求全部 L3 完成。
2. 「保存能力评级」「加入/移出提升计划草稿」「生成所选学习任务」是三个独立动作。
3. 已选择提升项持久化为计划草稿，可退出后继续；只有显式「生成所选学习任务」形成正式计划项/任务。
4. 计划时间只有一个 `plan_month=YYYY-MM`；无独立季度输入，不默认月份。整个月份输入区域可点击打开贴近输入框的年月弹层。季度如现有统计需要可由月份推导，但不作为用户动作或重点文案。
5. 行左侧勾选与右侧「加入提升计划」不能表达重复选择；最终用单一清晰的加入/移出动作。
6. 生成时所选 L3 缺计划月份则整批零写入、保留输入并逐项中文提示；未选 null 不阻断评级保存。
