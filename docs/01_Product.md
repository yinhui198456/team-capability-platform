...
6. Member 在能力自评/Gap 页面选择一或多个已填写的适用能力项（current_level 0～5，含 0），点击「生成所选学习任务」；未选择的能力项不阻塞，可后续增量选择其他能力项。
7. 系统在同一事务内校验所选能力项，并原子生成 Gap、年度成长计划（复用正式/可用年度计划）、计划项和学习任务；重复选择同一能力项幂等，不重复创建。
