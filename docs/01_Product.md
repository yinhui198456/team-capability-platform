...
6. Member 在能力自评/Gap 页面选择一或多个已填写的适用能力项（current_level 0～5，含 0），点击「生成所选学习任务」；未选择的能力项不阻塞，可后续增量选择其他能力项。每个选中能力项必须显式填写计划季度与计划月份；系统不提供也不应用任何默认季度/月份。
7. 系统在同一事务内校验所选能力项及其计划时间：任一选中项缺计划季度或月份时，整批拒绝，不生成也不复用任何计划项/学习任务，并逐项提示缺失内容与补齐方式；校验通过后原子生成 Gap、年度成长计划（复用正式/可用年度计划）、计划项和学习任务；重复选择同一能力项幂等，不重复创建。
...
Gap 在 Member 保存评级草稿时同步生成。学习任务仅通过显式「生成所选学习任务」入口创建：系统在单一事务中原子生成 Gap、复用/创建年度成长计划、计划项和学习任务（1:1）。提交自评的旧写路径已退役（#178）：不创建 Assessment Review、不改变状态、不生成或复用任何计划/任务。Buddy 的复核结论作为反馈与历史记录，不影响计划生成。
...
- Member 在能力自评/Gap 页面选择已填写的适用 L3（current_level 0～5，含 0）后，系统立即为所选 Gap 生成年度成长计划（复用正式/可用计划）及其计划项和学习任务（原子事务）；未选择项不阻塞（#178）。
...
Member 在能力自评/Gap 页面选择已填写的适用 L3 后，系统原子生成：Gap 分析 → 年度成长计划（复用正式/可用计划）→ 计划项（从被选 L3 的 Gap）→ 学习任务（1:1）。（#178）
...
- Member 选择已填写的适用 L3 后，系统原子生成：Gap 分析 → 年度成长计划 → 计划项（被选 L3 的 Gap）→ 学习任务（1:1）；未选择项不阻塞（#178）。
- 计划周期默认为 12 个月。
...
- 当前等级 1–2 级依据可为空；3–5 级生成所选学习任务时必须有有效依据。相对最近一次认可结果发生任何提升（包括 1→2）时，必须填写与继承依据不同的新依据；等级未变化且历史依据有效时可继续沿用。草稿允许暂缺依据，生成时对所选 L3 执行完整校验（#178）。
- 草稿支持稀疏保存：未提交的能力项保持原值；显式清空与未提交字段语义不同。生成所选学习任务时只校验被选 L3（#178）。
- Assessment 修改使用 revision/expected_revision 乐观并发控制；冲突返回 409，不静默覆盖。

## Issue #82 弱管理流程（原子生成）

- Member 显式生成所选学习任务时，系统在单一事务中原子生成：Gap 分析 → 年度成长计划 → 计划项（从标记为「纳入计划」的 Gap）→ 学习任务（1:1 对应）。提交自评写路径已退役（#178），评级仅通过草稿保存落盘。
...
- Issue #178 起：改为按所选能力项增量生成（选择已填写的适用 L3，批次原子、重复生成幂等），不再整份提交自评；本段为 Issue #82 历史记录。
