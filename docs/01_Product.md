...
4. Member 设定目标掌握度（0～5）。
5. 全部评估完成后，Member 提交自评结果。
6. 提交后的自评进入「待复核」状态，Buddy 可进行复核。

### 5.4 目标等级确定

- 目标掌握度由 Member 结合当前职级与年度成长目标自行设定。
- 默认建议：目标掌握度不低于当前职级对应基准，可向目标职级对齐。
- Buddy 可对不合理的目标掌握度提出调整建议。
...
- 平台保留每次评估的完整快照，包括评估时间、当前掌握度、目标掌握度、自评依据、Buddy 复核结论。
- Member 可查看个人能力成长曲线。
...
| 实时协同 | 多人同时编辑同一份评估或计划 | 采用简单乐观锁或最后写入优先策略。 |
