...
6. Member 在能力自评/Gap 页面通过「纳入年度计划」决策（是/否/未选择）持久化计划草稿选择；该项决策是唯一的选择来源（页面不再提供临时勾选列），随草稿保存并在刷新/重进后保持。点击「生成学习任务」时，系统只取所有「纳入年度计划=是」的能力项（否/未选择项永不进入请求，未纳入的能力项不阻塞生成）。每个纳入能力项必须显式填写计划月份（YYYY-MM，唯一可见的计划时间输入；季度为系统按月份派生的内部数据，不是第二个输入，系统不提供也不应用任何默认季度/月份）。
7. 系统在同一事务内校验纳入能力项及其计划时间：任一纳入项缺计划月份时，整批拒绝，不生成也不复用任何计划项/学习任务，并逐项提示缺失内容与补齐方式；校验通过后原子生成 Gap、年度成长计划（复用正式/可用年度计划）、计划项和学习任务；重复生成同一能力项幂等，不重复创建。
8. 评级保存与学习任务生成解耦：Member 可独立保存一或多个已填写 L3 的评级（current_level=0 是合法已填写值），保存本身不生成、不复用任何学习任务，也不创建 Assessment Review；未选择且未填写的 L3 不阻断评级保存。
...
- Member 按所选能力项生成学习任务时，系统不再创建 Assessment Review，也不进入 Buddy 自评复核队列；Buddy 不再承担新的自评审核/批准。
- Buddy 的职责是成果验收：对学习任务中 Member 提交的成果与证据进行 Evidence Review。
- 历史 Assessment Review 记录保持只读兼容，仅供追溯，不参与新流程。
...
Gap 在 Member 保存评级草稿时同步生成。学习任务仅通过显式「生成学习任务」入口创建：系统在单一事务中原子生成 Gap、复用/创建年度成长计划、计划项和学习任务（1:1）。计划草稿选择（纳入年度计划=是）持久化在草稿中，是唯一的选择来源；显式生成只处理纳入=是的行。提交自评的旧写路径已退役（#178）：不创建 Assessment Review、不改变状态、不生成或复用任何计划/任务。Buddy 的复核结论作为反馈与历史记录，不影响计划生成。
...
- Member 将 Gap > 0 的项标记为「纳入年度计划 = 是」，形成年度需要补齐的能力范围。
- 高优先级项建议默认纳入计划。
- 暂缓项不纳入计划，但保留在 Gap 列表中供后续参考。
- Member 在能力自评/Gap 页面将已填写的适用 L3（current_level 0～5，含 0）的「纳入年度计划」设为「是」（决策随草稿持久化）后，系统立即为所选 Gap 生成年度成长计划（复用正式/可用计划）及其计划项和学习任务（原子事务）；未纳入项不阻塞（#178）。
...
Member 在能力自评/Gap 页面将已填写的适用 L3 的「纳入年度计划」设为「是」后，系统原子生成：Gap 分析 → 年度成长计划（复用正式/可用计划）→ 计划项（从被选 L3 的 Gap）→ 学习任务（1:1）。（#178）
...
| 评级保存与任务生成 | 年初 | Member 保存评级草稿（评级独立落盘、同步生成 Gap）；按所选能力项显式生成学习任务，原子生成计划项与任务（#178，提交自评写路径已退役） | Member / 系统 |
| Buddy 复核 | 全年 | Buddy 对学习任务成果与证据进行 Evidence Review（自评不再进入 Buddy 复核队列，#178） | Buddy |
...
- Buddy 不再承担新的自评审核/批准；历史自评复核记录仅只读追溯（#178）。

### 13.3 计划规则

- Member 将已填写的适用 L3 的「纳入年度计划」设为「是」后，系统原子生成：Gap 分析 → 年度成长计划 → 计划项（被选 L3 的 Gap）→ 学习任务（1:1）；未纳入项不阻塞（#178）。
- 计划周期默认为 12 个月。
- 学习计划建议包含：状态为「已完成」、证据链接或文件说明、复盘结论。
- 生成学习任务不创建 Assessment Review，也不进入 Buddy 自评复核队列（#178）。
...
- Admin 拥有系统管理权限及全量数据查看权限；自评、计划维护、Evidence Review 等业务操作权限按 Member、Buddy、Leader 角色叠加。
...
- MVP 不建设独立消息中心。
- 待办提醒（待 Review Evidence）、计划到期提醒、学习任务逾期提醒仅在首页看板展示。
- 不依赖外部 IM / 邮件 / 短信系统。
...
- 当前等级 1–2 级依据可为空；3–5 级生成学习任务时必须有有效依据。相对最近一次认可结果发生任何提升（包括 1→2）时，必须填写与继承依据不同的新依据；等级未变化且历史依据有效时可继续沿用。草稿允许暂缺依据，生成时对纳入计划 L3 执行完整校验（#178）。
- 草稿支持稀疏保存：未提交的能力项保持原值；显式清空与未提交字段语义不同。生成学习任务时只校验纳入计划（include_in_plan=是）的 L3（#178）。
- Assessment 修改使用 revision/expected_revision 乐观并发控制；冲突返回 409，不静默覆盖。

## Issue #82 弱管理流程（原子生成）

- Member 显式生成学习任务时，系统在单一事务中原子生成：Gap 分析 → 年度成长计划 → 计划项（从标记为「纳入计划」的 Gap）→ 学习任务（1:1 对应）。「纳入计划」决策持久化在草稿中且是唯一的选择来源；提交自评写路径已退役（#178），评级仅通过草稿保存落盘。
...
- Issue #178 起：改为按所选能力项增量生成（将已填写的适用 L3 的「纳入年度计划」设为「是」，批次原子、重复生成幂等），不再整份提交自评；本段为 Issue #82 历史记录。
