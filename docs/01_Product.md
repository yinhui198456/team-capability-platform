...
7. 提交后的自评立即触发 Gap 分析、年度成长计划、计划项和学习任务的原子生成。Buddy 可查看标准目标、个人调整及原因并进行复核（复核结果作为历史记录与反馈）。

> ⚠️ 本步骤的原子生成与复核流程已由 Issue #187 故事合同替代（独立动作 + 无新自评复核队列），见「Issue #187 故事合同」。
...
> ⚠️ 本节已由 Issue #187 故事合同第 7 条替代：不再创建新的 Assessment Review/Buddy 自评复核队列；历史 Assessment Review 记录只读。Buddy Evidence Review 保留。
...
- Member 提交自评时，系统立即执行原子事务：生成 Gap、创建年度成长计划、生成计划项和学习任务。Buddy 复核结论作为反馈与历史记录，不阻塞计划生成。
...
Gap 在 Member 提交自评后立即生成；提交时系统原子生成 Gap、年度成长计划、计划项和学习任务。Buddy 的复核结论作为反馈与历史记录，不影响计划生成。

> ⚠️ 提交时原子生成已由 Issue #187 故事合同第 2、3、6 条替代；Buddy 自评复核由第 7 条替代（历史只读）。
...
> ⚠️ 本节已由 Issue #187 故事合同第 2、3、5、6 条替代：提升项持久化为计划草稿，仅显式「生成所选学习任务」形成正式计划项/任务，使用单一加入/移出动作。

- Member 将 Gap > 0 的项标记为「是否纳入计划 = 是」，形成年度需要补齐的能力范围。
- 高优先级项建议默认纳入计划。
- 暂缓项不纳入计划，但保留在 Gap 列表中供后续参考。
- Member 提交自评时，系统立即将标记为「纳入计划」的 Gap 项生成年度成长计划及其计划项和学习任务（原子事务）。
...
> ⚠️ 本节已由 Issue #187 故事合同第 2、3 条替代：提升项先持久化为计划草稿，只有显式「生成所选学习任务」才形成正式计划项/学习任务（1:1）。

Member 提交自评时，系统原子生成：Gap 分析 → 年度成长计划 → 计划项（从标记为「纳入计划」的 Gap）→ 学习任务（1:1）。
...
> ⚠️ 本节时间字段已由 Issue #187 故事合同第 4 条替代：计划时间仅 `plan_month=YYYY-MM`，无独立季度输入、不默认月份；计划开始/截止日期、目标月份与季度边界不再作为用户输入。
...
> ⚠️ 计划时间字段已由 Issue #187 故事合同第 4 条替代：仅 `plan_month=YYYY-MM` 可调整。
...
> ⚠️ 自评复核职责已由 Issue #187 故事合同第 7 条替代：不再创建新的自评复核队列，历史 Assessment Review 只读；Buddy Evidence Review 与进度指导保留。
...
自评提交后，系统立即生成成长计划；Buddy 复核结果作为反馈，不阻塞计划生成。
...
| 自评提交 | 年初 | Member 提交自评，系统原子生成 Gap、年度成长计划、计划项和学习任务 | Member / 系统 |
| Buddy 复核 | 年初 | Buddy 复核 Member 自评结果，给出反馈（不阻塞计划生成） | Buddy |
...
3. Member 在规定周期内完成自评并提交；提交时系统原子生成 Gap 分析、年度成长计划、计划项和学习任务。

> ⚠️ 本步骤的提交时原子生成已由 Issue #187 故事合同第 2、3 条替代（独立动作 + 显式生成）。

4. Buddy 在自评提交后完成复核，给出反馈（复核结果不影响已生成的计划）。
5. Member 根据已生成的计划和 Buddy 反馈开始执行学习任务。
...
> ⚠️ 本节首条已由 Issue #187 故事合同第 2、3 条替代：提升项持久化为计划草稿，仅显式「生成所选学习任务」形成正式计划项/任务。

- Member 提交自评时，系统原子生成：Gap 分析 → 年度成长计划 → 计划项（标记为「纳入计划」的 Gap）→ 学习任务（1:1）。
- 计划周期默认为 12 个月。
- 学习计划建议包含：状态为「已完成」、证据链接或文件说明、复盘结论。
- Buddy 复核结果作为反馈，不影响计划生成。
...
- Buddy 对 Member 自评进行复核并给出反馈；复核结果不阻塞计划生成。
- Buddy Review Member 提交的 Evidence，给出通过、需补充或驳回结论。
- Buddy 提供指导、反馈，不是审批人。
...
- Member 提交自评时，系统在单一事务中原子生成：Gap 分析 → 年度成长计划 → 计划项（从标记为「纳入计划」的 Gap）→ 学习任务（1:1 对应）。
- Buddy 复核结果作为反馈与历史记录，不阻塞计划生成。
- 原子生成幂等：重复提交时，已存在的计划项不再重复创建；仅为新增 Gap 项创建计划项和学习任务。
- API 响应格式新增 `plan_generation` 字段：`{"annual_plan_id": int, "created_items": int, "skipped_items": int, "created_tasks": int}`。

> ⚠️ 本节「提交自评时原子生成计划与任务」已由 Issue #187 故事合同及 Issue #201 M02 更新替代：评级自动保存、计划草稿自动保存、显式「生成所选学习任务」相互独立，见下文「Issue #187 故事合同」。

## Issue #187 故事合同（第一批原型基线，2026-08-16 确认）

本节记录 Issue #187 阶段 0 确认的故事合同（原型基线）；其中 M02 第 1～6 条已按 Issue #201 阶段 1 确认更新为下述现行合同。与上文既有规则冲突时，以本节为准；上文相关段落已就地标注「已替代」，原文保留供历史追溯。原型资产与打开方式见 `docs/assets/ui-prototypes/README.md`；页面矩阵见 `docs/04_UI.md` §4.9。

### 页面批次与最终版本

- **第一批（原型已定版，共 9 页）**：M01 我的工作台 V1、M02 能力评级与提升计划 V1、M03 年度成长计划 V1、M04 学习任务 V1、M05 学习任务详情 V1、B01 成果验收 V2（仅 Evidence/成果验收，不恢复 Buddy 自评复核）、D01 成员学习进度看板 V3（统一浅色调；原型数字仅为候选指标，非业务口径）、L01 团队能力分析 V1、A01 用户、角色与辅导关系 V1（含新增用户状态与用户等级设置）。
- **第二批**：M06 月度复盘、M07 成长档案。
- **不实施**：L02 年度能力重点，移出实施范围。

### 故事合同（M02 已按 Issue #201 更新）

1. 能力评级允许任意部分逐项选择；每次选择自动保存并清楚反馈保存中、已保存或失败，不要求全部 L3 完成，也不显示「保存能力评级」按钮。
2. 评级自动保存、加入/移出及编辑计划草稿自动保存、显式「生成所选学习任务」相互独立。
3. 已选择提升项持久化为计划草稿，可退出后继续；只有显式「生成所选学习任务」形成正式计划项/任务。
4. 加入提升计划时优先级默认「低」，可选调整为「中/高」，不作为生成门禁；计划时间只有一个必填的 `plan_month=YYYY-MM`，无独立季度输入且不默认月份。整个月份输入区域可点击打开贴近输入框的年月弹层。季度如现有统计需要可由月份推导，但不作为用户动作或重点文案。
5. 行左侧勾选与右侧「加入提升计划」不能表达重复选择；最终用单一清晰的加入/移出动作。
6. 生成仅在未选择任何 L3 或所选 L3 缺计划月份时阻断；缺月份时整批零写入、保留输入并逐项中文提示，且定位到首个缺失月份。
7. 不创建新的 Assessment Review/Buddy 自评队列；历史 Assessment Review 只读；Buddy Evidence Review 保留。
8. 任务生成后若对应 L3 的预期输出、输出类型等要求变化，通知 Member，并允许「采用新要求」或「按原任务要求继续」；普通评级/目标小幅变化不强制重建。
9. Leader/Buddy 需要成员学习进度看板，可按周/月查看；本周学习时间、总学习时间、任务数、完成数、延期数只是候选指标，独立 Issue 确认口径后才能实施。
10. 采用小型公共 UI 规则：标题/说明/主操作、侧栏与内容宽度、按钮/输入/年月选择/只读字段、中文状态、保存/失败/空态/加载反馈、表格/卡片/抽屉/弹窗场景、1440/1024/768 基本行为；不建复杂设计系统，不直接显示内部英文状态。
11. 生成成功后在生成操作旁提供当年度「查看学习任务」入口；不自动跳转或清空当前评级与计划草稿。

### 原型定位

- 原型是交互与信息架构基线，绝不自动定义不存在的 API、指标口径或数据库字段；不连接后端。
- 自动打开检查与人工视觉验收是两个不同层面：阶段 0 只完成静态资产的打开/渲染检查，不声明业务 UAT。

### 被替代的旧规则（原文保留，不再作为权威）

- 「提交自评时原子生成 Gap → 年度成长计划 → 计划项 → 学习任务」（§5.3、§5.6、§6.1、§6.4、§7.1、§12.2、§12.3、§13.3 及 Issue #82 相关段落）→ 由故事合同第 2、3、6 条替代。
- Buddy 对自评的复核流程（§5.6、§10.2）→ 由故事合同第 7 条替代：不再创建新的 Assessment Review/Buddy 自评队列，历史记录只读。
- 计划时间采用计划开始/截止日期、目标月份与季度边界（§7.4、§7.7）→ 由故事合同第 4 条替代：仅 `plan_month=YYYY-MM`。
