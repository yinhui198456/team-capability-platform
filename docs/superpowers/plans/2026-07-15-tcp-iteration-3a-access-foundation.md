...
**Architecture:** A new `backend/app/access/` module uses raw psycopg 3 for schema, repository, and policy. FastAPI lifespan creates the access schema and seeds a fixed set of UAT-only demo accounts when `tcp_user` is empty. Authentication is stateful: the backend issues a random session token stored in `tcp_session`, returns it only via `Set-Cookie: tcp_session=...; HttpOnly; Secure; SameSite=Lax`, and never exposes the token in JSON or localStorage. Catalog routes stay unprotected; new protected demo routes prove 401/403. The frontend adds a minimal `/login` page used only for UAT, leaving full UI-01/02/03/04/05 integration to later iterations.
...
| `backend/app/settings.py` | Add `session_secret: str`, `session_max_age_seconds: int = 86400`, `demo_account_passwords: dict[str, str] | None = None`, and keep existing `database_url`, `cors_origins`, `port`. |
...
```sql
CREATE INDEX IF NOT EXISTS idx_session_token ON tcp_session(token);
...
- `current_user` reads `tcp_session` from the `tcp_session` cookie, validates expiry, and returns the user dict with `roles` list.
...
|--------|-------|------|---------|----------|
| `POST` | `/api/auth/login` | public | `{"username": "...", "password": "..."}` | `200` + `Set-Cookie: tcp_session=...; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=<seconds>` + body `{"id": ..., "username": "...", "email": "...", "full_name": "...", "roles": ["..."]}` |
| `POST` | `/api/auth/logout` | authenticated | — | `200` + `Set-Cookie: tcp_session=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0` + body `{"ok": true}` |
| `GET`  | `/api/auth/me` | authenticated | — | `200` + `{"id": ..., "username": "...", "email": "...", "full_name": "...", "roles": ["..."], "primary_buddy": {...} | null, "assigned_members": [...]}` |

- Login failures return `401` with `{"detail": "invalid credentials"}`.
- The `tcp_session` cookie value is never returned in the JSON body.

### Protected demo route (for tests only, may be removed in 3B)

```python
@router.get("/api/demo/protected")
async def protected_demo(user: CurrentUser) -> dict[str, object]: ...

@router.get("/api/demo/leader-only")
async def leader_only_demo(user: UserWithRole("Leader")) -> dict[str, object]: ...
```

These routes prove 401/403 and are documented as temporary; they are not part of the product API.
...
`backend/app/access/seed.py` seeds the following accounts only when `tcp_user` is empty at lifespan startup. Passwords are read from the environment variable `TCP_DEMO_PASSWORDS` as JSON, e.g.:

```bash
TCP_DEMO_PASSWORDS='{"admin":"admin","leader":"leader","buddy":"buddy","member":"member","member2":"member2"}'
```

If the environment variable is absent or malformed, a deterministic fallback is used for local/UAT only (not production). The seed hashes passwords via `hash_password` before insertion.
...
- [ ] **Implement:** implement `security.py` with `hashlib.scrypt` and `secrets`; implement `repository.py` with parameterized psycopg queries; add `session_max_age_seconds` to settings.
...
- [ ] **Implement:** build `policies.py` and `api.py`; wire the router in `main.py`; change CORS `allow_methods` to `["GET", "POST", "OPTIONS"]`; set `Secure` only when the request is HTTPS or via a setting, defaulting to `Secure` for production and allowing tests to override.
...
**Interfaces:**
- Temporary protected routes `/api/demo/protected` and `/api/demo/leader-only`.

- [ ] **RED:** add tests for a user with only `Member` accessing `/api/demo/leader-only` and receiving 403, a user with `Leader` accessing it successfully, and an `Admin` without `Leader` receiving 403. Run `docker compose run --rm -v "$PWD/backend:/workspace/backend:ro" -v "$PWD/capability-model:/capability-model:ro" backend sh -c "pip install -q -r /workspace/backend/requirements-dev.txt && cd /workspace/backend && PYTHONPATH=/workspace/backend pytest -q tests/test_access_roles.py"`; expect 403 logic missing.
- [ ] **Implement:** complete `require_any_role`; add the two demo routes to `api.py` marked as temporary.
...
- [ ] **RED:** add tests that creating a second active primary Buddy for the same Member raises a unique violation, that `member` resolves `buddy` as primary Buddy, and that `buddy` lists `[member, member2]`. Run `docker compose run --rm -v "$PWD/backend:/workspace/backend:ro" -v "$PWD/capability-model:/capability-model:ro" backend sh -c "pip install -q -r /workspace/backend/requirements-dev.txt && cd /workspace/backend && PYTHONPATH=/workspace/backend pytest -q tests/test_access_buddy.py"`; expect repository not yet used.
- [ ] **Implement:** ensure the partial unique index in schema.py is created; verify repository functions return the correct shape.
...
- [ ] **Implement:** create `seed.py` and call it from lifespan after schema creation; read `TCP_DEMO_PASSWORDS` via settings.
...
# 4. UAT smoke: login returns HttpOnly cookie
...
- [ ] `tcp_session` cookie attributes include `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/`.
