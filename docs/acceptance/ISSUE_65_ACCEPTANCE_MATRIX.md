...
3. 本文件创建轮为纯文档工作：未运行任何构建、测试、浏览器、容器、数据库命令、迁移、备份、恢复或快照。
...
| ID | 范围 | 身份/数据前置 | 操作 | 预期结果 | 状态 | 证据 |
|---|---|---|---|---|---|---|
| UAT-65-M | 用户 UAT：Member | 用户提供的 UAT 业务账号与代表性职级组合 | Member 视角验收：能力地图职级标准、自评范围非固定 310、必备/进阶分类、主表 7 字段、0–5/优先级/计划时间填写、无依据列与普遍依据门禁、提交与错误提示 | 用户明确通过 | NOT RUN | |
...
### 6.2 必须获得用户**新鲜明确授权**后方可执行的动作

- 真实 UAT 部署；
- UAT 凭证 / 业务账号的提供、确认与使用；
- 数据库备份、恢复、清理；
- 破坏性迁移 / 回滚操作；
- PR Ready、合并（merge）；
- 关闭任何 Issue；
...
本轮为纯文档新增（仅本文件）。仓库未提供已文档化的轻量 markdown/文档检查命令（CI 仅有 backend / frontend / e2e 三套工作流），因此**无适用的自动化测试可运行**；未运行任何构建、测试、容器或数据库命令。
