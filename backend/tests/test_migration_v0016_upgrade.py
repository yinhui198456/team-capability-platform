...
def test_v0016_allows_later_same_member_year_and_preserves_scope_guard(
    ...
    wrong_year_assessment, wrong_year_detail = _new_assessment_detail(
        connection,
        member_id=int(data["member_id"]),
        year=2025,
        l3_node_id=int(data["l3_node_id"]),
        code="P01-L1-L2-L3-WRONG-YEAR",
    )
    with pytest.raises(psycopg.errors.ForeignKeyViolation) as wrong_year:
        with connection.transaction():
            _insert_item(
                connection,
                plan_id=int(data["plan_id"]),
                assessment_id=wrong_year_assessment,
                detail_id=wrong_year_detail,
                l3_node_id=int(data["l3_node_id"]),
                code="P01-L1-L2-L3-WRONG-YEAR",
            )
    assert wrong_year.value.diag.constraint_name == (
        "plan_item_source_member_year_guard"
    )
