...
- CI with no run or empty steps/logs proves nothing: verify owner/billing, visibility/Actions budget, Actions enabled, runner/concurrency, event and base/path filters, and existing exact-SHA attempts first; switching accounts changes permissions only, never Actions billing; resume at most once per exact SHA per workflow after a proven trigger gap or changed condition; no empty commits, guess-fixes, or CI-only PR bypasses (full text in `.claude/rules/delivery-boundaries.md`).
- Test ladder: cosmetic/test-only → red test + targeted/affected + same-SHA CI; ordinary page state/timing → component red test + one real API E2E + same-SHA CI; only structural families (migration/auth/concurrency/idempotency/harness/contract) get one clean local full gate (full text in `.claude/rules/testing-and-evidence.md`).
