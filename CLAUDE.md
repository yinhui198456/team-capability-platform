...
- Delivery memory — when setting up or restoring an Issue, changing stages, or handing off, read `.agents/tcp-delivery-memory.md`「七阶段交付与反馈回退」as needed (derived from #201); it never expands authority, which still comes from GitHub and the current bounded contract.
...
- CI with no run or empty steps/logs proves nothing: verify owner/billing, visibility/Actions budget, Actions enabled, runner/concurrency, event and base/path filters, and existing exact-SHA attempts first; switching accounts changes permissions only, never Actions billing; retry or dispatch at most once per exact SHA per workflow after a proven trigger gap or changed condition; no empty commits, guess-fixes, or CI-only PR bypasses (full text in `.claude/rules/delivery-boundaries.md`).
- Test ladder: cosmetic/test-only — behavior/contract defects need a check/regression failing on the old version, pure docs/formatting need exact checker + diff review; both targeted/affected with same-SHA CI as full gate; page state/timing → component red test + one real API E2E + same-SHA CI; only structural families get one clean local full gate (full text in `.claude/rules/testing-and-evidence.md`).
- UAT admission: canonical deployment entry + protected config reference only; record a non-sensitive rotation/report reference before writes; probes must pass; never record credential values (full text in `.claude/skills/tcp-uat-execution/SKILL.md`).
- Delivery conclusions stay in four separate layers — an integration-branch merge is not a master or production release (full text in `.claude/rules/delivery-boundaries.md`).
